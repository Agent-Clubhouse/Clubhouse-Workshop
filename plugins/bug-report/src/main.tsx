...
// ---------------------------------------------------------------------------
// MainPanel — split layout: issue list on left, detail on right
// ---------------------------------------------------------------------------

function ContentPane({ api }: PanelProps) {
  const [, setTick] = useState(0);
  const rerender = useCallback(() => setTick(t => t + 1), []);
    ...
    <div style={S.main}>
      <div style={S.empty}>
        ...
        <div style={{ fontSize: "12px", maxWidth: "300px" }}>
          File a bug report or feature request, or select an existing report from the list.
        </div>
        <button
