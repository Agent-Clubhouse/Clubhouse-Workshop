...
function createMockAgents(): AgentsAPI {
  return {
    ...
    listSessions: createMockFn().mockResolvedValue([]) as unknown as AgentsAPI["listSessions"],
    readSessionTranscript: createMockFn().mockResolvedValue(null) as unknown as AgentsAPI["readSessionTranscript"],
    getSessionSummary: createMockFn().mockResolvedValue(null) as unknown as AgentsAPI["getSessionSummary"],
    spawnCompanion: createMockFn().mockResolvedValue("companion-agent-id") as unknown as AgentsAPI["spawnCompanion"],
    getCompanionStatus: createMockFn().mockResolvedValue("none") as unknown as AgentsAPI["getCompanionStatus"],
    getCompanionWorkspace: createMockFn().mockResolvedValue("/mock/workspace") as unknown as AgentsAPI["getCompanionWorkspace"],
  };
}
...
function createMockWindow(): WindowAPI {
  let currentTitle = "";
  return {
    setTitle: createMockFn().mockImplementation((title: unknown) => {
      currentTitle = String(title);
    }) as unknown as WindowAPI["setTitle"],
    resetTitle: createMockFn().mockImplementation(() => {
      currentTitle = "";
    }) as unknown as WindowAPI["resetTitle"],
    getTitle: createMockFn().mockImplementation(() => currentTitle) as unknown as WindowAPI["getTitle"],
  };
}

function createMockCanvas(): CanvasAPI {
  return {
    registerWidgetType: createMockFn().mockReturnValue(noop) as unknown as CanvasAPI["registerWidgetType"],
    queryWidgets: createMockFn().mockReturnValue([]) as unknown as CanvasAPI["queryWidgets"],
  };
}

function createMockMcp(): McpAPI {
  return {
    contributeTools: createMockFn().mockResolvedValue(undefined) as unknown as McpAPI["contributeTools"],
    removeTools: createMockFn().mockResolvedValue(undefined) as unknown as McpAPI["removeTools"],
    listContributedTools: createMockFn().mockResolvedValue([]) as unknown as McpAPI["listContributedTools"],
    onToolCall: createMockFn().mockReturnValue(noop) as unknown as McpAPI["onToolCall"],
  };
}
