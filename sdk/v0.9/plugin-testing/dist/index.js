...
__export(index_exports, {
  ALL_SOUND_EVENTS: () => ALL_SOUND_EVENTS,
...
function createMockFiles() {
  const filesApi = {
    ...
    search: createMockFn().mockResolvedValue({ results: [], totalMatches: 0, truncated: false })
  };
...
function createMockAgents() {
  return {
    ...
    listSessions: createMockFn().mockResolvedValue([]),
    readSessionTranscript: createMockFn().mockResolvedValue(null),
    getSessionSummary: createMockFn().mockResolvedValue(null),
    spawnCompanion: createMockFn().mockResolvedValue("companion-agent-id"),
    getCompanionStatus: createMockFn().mockResolvedValue("none"),
    getCompanionWorkspace: createMockFn().mockResolvedValue("/mock/workspace")
  };
}
function createMockUI() {
  return {
    ...
    showApprovalDialog: createMockFn().mockResolvedValue(null),
...
function createMockAgentConfig() {
  return {
    ...
    contributeWrapperPreset: createMockFn().mockResolvedValue(void 0)
  };
}
...
function createMockWindow() {
  let currentTitle = "";
  return {
    setTitle: createMockFn().mockImplementation((title) => {
      currentTitle = String(title);
    }),
    resetTitle: createMockFn().mockImplementation(() => {
      currentTitle = "";
    }),
    getTitle: createMockFn().mockImplementation(() => currentTitle)
  };
}
function createMockCanvas() {
  return {
    registerWidgetType: createMockFn().mockReturnValue(noop),
    queryWidgets: createMockFn().mockReturnValue([])
  };
}
function createMockMcp() {
  return {
    contributeTools: createMockFn().mockResolvedValue(void 0),
    removeTools: createMockFn().mockResolvedValue(void 0),
    listContributedTools: createMockFn().mockResolvedValue([]),
    onToolCall: createMockFn().mockReturnValue(noop)
  };
}
